...
server.post("/photos/", function (req, res, next) {
  const error = validatePhoto(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.title); // Generate a slug for new photos.
    next();
  }
});
...
function validatePhoto(photo) {
  if (!photo.title) return "Title is required.";
  if (!photo.authorId) return "Author is required.";
  if (!photo.category) return "Category is required.";
  return "";
}
